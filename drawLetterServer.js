...
io.on('connection', function(socket){
    socket.emit('sendPts',getValues(x,y))
...
function openConnection(i, data, socket){
    ...
    var client = net.connect({port: 5002, host:"130.64.92.39"});
...
//takes in a (x,y) for pen location, returns 4 robot angles in an array
    ...
    var alpha = Math.atan2(l1-(z+l5),r);
    ...
    var theta3deg = theta3*180/pi;
    var theta4 = pi/2 - (theta2+theta3);
    var theta4deg = theta4*180/pi;
    var angles=new Array(4);
    angles[0]=[theta1,theta1deg];
    angles[1]=[theta2,-1*theta2deg];
    angles[2]=[theta3,-1*theta3deg];
    angles[3]=[theta4,-1*theta4deg];
//    console.log(angles)
    return angles
}
